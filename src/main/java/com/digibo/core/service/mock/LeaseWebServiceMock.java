package com.digibo.core.service.mock;

import com.digibo.core.service.LeaseWebService;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.context.annotation.Profile;
import org.springframework.stereotype.Service;

import java.util.*;

/**
 * LeaseWebServiceMock - Mock implementation of LeaseWebService
 * Returns test data without database calls
 */
@Service
@Profile("mock")
public class LeaseWebServiceMock implements LeaseWebService {

    private static final Logger logger = LoggerFactory.getLogger(LeaseWebServiceMock.class);

    private final List<Map<String, Object>> mockOrders = new ArrayList<>();

    public LeaseWebServiceMock() {
        initMockData();
    }

    private void initMockData() {
        Map<String, Object> order1 = new HashMap<>();
        order1.put("PID", "LW001");
        order1.put("CUSTID", "CUST001");
        order1.put("CUSTOMER_NAME", "John Doe");
        order1.put("LOGIN", "jdoe");
        order1.put("STATUS", 60);
        order1.put("CLASS_ID", 20);
        order1.put("CREATED", new Date());
        order1.put("PROCESSED_BY", null);
        order1.put("LEGAL_ID", "123456-12345");
        mockOrders.add(order1);

        Map<String, Object> order2 = new HashMap<>();
        order2.put("PID", "LW002");
        order2.put("CUSTID", "CUST002");
        order2.put("CUSTOMER_NAME", "Jane Smith");
        order2.put("LOGIN", "jsmith");
        order2.put("STATUS", 61);
        order2.put("CLASS_ID", 21);
        order2.put("CREATED", new Date());
        order2.put("PROCESSED_BY", 1001L);
        order2.put("LEGAL_ID", "654321-54321");
        mockOrders.add(order2);
    }

    @Override
    public List<Map<String, Object>> find(String custId, String custName, String userLogin,
                                           String docId, String statuses, String docClass,
                                           Date createdFrom, Date createdTill,
                                           String customerName, String legalId) {
        logger.debug("[MOCK] find()");

        return mockOrders.stream()
                .filter(o -> custId == null || custId.equals(o.get("CUSTID")))
                .filter(o -> custName == null || ((String) o.get("CUSTOMER_NAME")).toLowerCase().contains(custName.toLowerCase()))
                .filter(o -> userLogin == null || ((String) o.get("LOGIN")).toLowerCase().contains(userLogin.toLowerCase()))
                .filter(o -> docId == null || docId.equals(o.get("PID")))
                .filter(o -> customerName == null || ((String) o.get("CUSTOMER_NAME")).toLowerCase().contains(customerName.toLowerCase()))
                .filter(o -> legalId == null || ((String) o.get("LEGAL_ID")).contains(legalId))
                .toList();
    }

    @Override
    public List<Map<String, Object>> findMy(Long officerId) {
        logger.debug("[MOCK] findMy({})", officerId);

        if (officerId == null || officerId == 0L) {
            return mockOrders.stream()
                    .filter(o -> o.get("PROCESSED_BY") == null)
                    .toList();
        } else {
            return mockOrders.stream()
                    .filter(o -> officerId.equals(o.get("PROCESSED_BY")))
                    .toList();
        }
    }

    @Override
    public Map<String, Object> amexorder(String orderId) {
        logger.debug("[MOCK] amexorder({})", orderId);

        Map<String, Object> response = new HashMap<>();
        response.put("id", orderId);
        response.put("userName", "Mock User");
        response.put("legalId", "000000-00000");
        response.put("officerName", null);
        response.put("custName", "Mock Customer");
        response.put("globusNo", "MOCK" + orderId);
        response.put("location", "LV");
        response.put("fromAccount", "LV00MOCK0000000000001 EUR");
        response.put("utPhoneNumber", "+371 20000000");
        response.put("phoneMobile", "+371 29000000");
        response.put("authName", "Mock");
        response.put("authSurname", "Authorized");
        response.put("authLegalId", "000000-00000");
        response.put("authPassportNo", "LV0000000");
        response.put("authPassportCountry", "LV");
        response.put("authPassportInst", "PMLP");
        response.put("authPhone", "+371 20000000");
        response.put("authFax", null);
        response.put("authEmail", "mock@example.com");
        response.put("infoToCustomer", "Mock order information");
        response.put("infoToBank", "Mock internal notes");
        response.put("channelId", 5);
        response.put("signTime", new Date());
        response.put("signDevType", 5);
        response.put("signDevId", "MOCK_DEVICE");
        response.put("signKey1", "MOCK_KEY1");
        response.put("signKey2", "MOCK_KEY2");
        response.put("signRSA", "MOCK_RSA_SIGNATURE");
        return response;
    }

    @Override
    public Map<String, Object> setProcessing(String orderId, Long statusIdFrom) {
        logger.debug("[MOCK] setProcessing({}, {})", orderId, statusIdFrom);

        Optional<Map<String, Object>> orderOpt = mockOrders.stream()
                .filter(o -> orderId.equals(o.get("PID")))
                .findFirst();

        Map<String, Object> response = new HashMap<>();
        if (orderOpt.isPresent()) {
            Map<String, Object> order = orderOpt.get();
            Integer status = (Integer) order.get("STATUS");
            if (statusIdFrom == null || statusIdFrom.intValue() == status) {
                order.put("PROCESSED_BY", 1001L);
                order.put("STATUS", 61);
                response.put("success", true);
                response.put("orderId", orderId);
                response.put("result", 1001);
                return response;
            }
        }

        response.put("success", false);
        response.put("orderId", orderId);
        response.put("result", 0);
        return response;
    }
}
